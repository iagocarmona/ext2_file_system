...
typedef struct ext2_super_block {
	...
	uint32_t	s_first_data_block;	/* primeiro bloco de dadps */
	uint32_t	s_log_block_size;	/* tamanho do bloco */
	uint32_t	s_log_frag_size;	/* tamnaho do fragmento */
	uint32_t	s_blocks_per_group;	/* blocos por grupo */
	uint32_t	s_frags_per_group;	/* fragmentos por grupo */
	uint32_t	s_inodes_per_group;	/* inodes popr grupo */
	...
	uint16_t	s_errors;		/* comportamento caso um erro seja detectado */
	...
	uint32_t	s_creator_os;		/* ID do sistema operacional */
...
typedef struct ext2_group_desc {
	...
	uint16_t	bg_free_blocks_count;	/* número total de blocos livres no grupo*/
...
typedef struct ext2_inode {
	...
	uint32_t	i_size;		/* time em bytes */
	uint32_t	i_atime;	/* hora de acesso */
	uint32_t	i_ctime;	/* hora de criação */
	uint32_t	i_mtime;	/* hora de modificação */
	uint32_t	i_dtime;	/* hora de exclusão */
	uint16_t	i_gid;		/*ID do grupo*/
	uint16_t	i_links_count;	/* número de links */
	uint32_t	i_blocks;	/* número de blocos */
	...
	uint32_t	i_block[EXT2_N_BLOCKS];/* ponteiros para blocos */
	...
	union {
		struct {
			...
			uint16_t	l_i_gid_high;	/* ID de grupo */
			uint32_t	l_i_reserved2; /* reservado */
		} linux2;
		struct {
			uint8_t	h_i_frag;	/* número de gragmento */
			uint8_t	h_i_fsize;	/* tamanho de fragmento */
		...
		struct {
			uint8_t	m_i_frag;	/* número de fragmento */
			uint8_t	m_i_fsize;	/* tamanho de fragmento */
			uint16_t	m_pad1;
